...
def work(month):
    ...
    string = 'category_train_parameter_' + month + '_'
    filename = ''
    for x in files:
        if string in x:
            filename = x
    ...
    m = len(training_data) + len(mix_data)
    for tt in range(num_of_try):
        m2 = int(m * (len(training_data) / (len(training_data) + len(mix_data)))) * times
        # m2 = m * times
        # print(m)
        print(tt)
        ...
        output = open(dir_path + "/K=T_order_rank_" + month + "_" + str(tt) + ".txt", "w")
